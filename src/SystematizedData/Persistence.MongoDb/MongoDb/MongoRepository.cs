...
public class MongoRepository : IRepositoryInternal
{
    ...
    public IDatabaseCollection<TKey, T> GetCollection<TKey, T>(IMongoDataSourceMapper<TKey, T> mongoDataSourceMapper, string? suffix = null)
        where TKey : notnull 
        where T : class, new()
    {
        var name = GetCollectionName<T>(suffix);

        return new DatabaseCollection<TKey, T>(_database.GetCollection<T>(name), mongoDataSourceMapper);
    }
