...
internal class TenantRepository : RuntimeRepositoryBase, ITenantRepository
{
    ...
    protected override async Task DeleteManyRtEntitiesAsync<TEntity>(IOctoSession session, CkId<CkTypeId> ckTypeId, ICollection<FieldFilter> fieldFilters)
    {
        var mutation = new Mutation<TEntity>(BulkRtMutation, _databaseContext);
        mutation.AddFieldFilters(fieldFilters);
        await mutation.DeleteManyAsync(session, ckTypeId).ConfigureAwait(false);
    }

    protected override async Task DeleteOneRtEntityAsync<TEntity>(IOctoSession session, CkId<CkTypeId> ckTypeId, ICollection<FieldFilter> fieldFilters)
    {
        var mutation = new Mutation<TEntity>(BulkRtMutation, _databaseContext);
        mutation.AddFieldFilters(fieldFilters);
    ...
    protected override async Task UpdateOneRtEntityAsync<TEntity>(IOctoSession session, CkId<CkTypeId> ckTypeId, ICollection<FieldFilter> fieldFilters, TEntity rtEntity)
    {
        var mutation = new Mutation<TEntity>(BulkRtMutation, _databaseContext);
        mutation.AddFieldFilters(fieldFilters);
    ...
    protected override async Task UpdateManyRtEntityAsync<TEntity>(IOctoSession session, CkId<CkTypeId> ckTypeId, ICollection<FieldFilter> fieldFilters, TEntity rtEntity)
    {
        var mutation = new Mutation<TEntity>(BulkRtMutation, _databaseContext);
        mutation.AddFieldFilters(fieldFilters);
        ...
        DataQueryOperation dataQueryOperation, int? skip = null, int? take = null)
    {
        ...
        var query = new CkTypeQuery(_databaseContext);
        query.AddFieldFilters(dataQueryOperation.FieldFilters);
    ...
    public IUpdateStream<RtEntity> SubscribeToRtEntities(CkId<CkTypeId> ckTypeId, UpdateStreamFilter updateStreamFilter,
        CancellationToken cancellationToken = default)
    {
        var collection = _databaseContext.GetRtDatabaseCollection<RtEntity>(ckTypeId);
        ...
        where TEntity : RtEntity, new()
    {
        ...
        var collection = _databaseContext.GetRtDatabaseCollection<TEntity>(ckTypeId);
    ...
    public IUpdateStream<RtAssociation> SubscribeToRtAssociations(CkId<CkTypeId> originCkTypeId, CkId<CkTypeId> targetCkTypeId,
        ...
        CancellationToken cancellationToken = default)
    {
        return _databaseContext.RtDatabaseAssociations.Subscribe(updateStreamFilter.UpdateTypes, () =>
        {
        ...
        string attributeName, string regexFilterValue, int takeCount)
    {
        ...
        var collection = _databaseContext.GetRtDatabaseCollection<RtEntity>(entityCacheItem.CkTypeId);
