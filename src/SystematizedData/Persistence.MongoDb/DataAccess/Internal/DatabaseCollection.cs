...
internal class DatabaseCollection<TKey, TDocument> : IDatabaseCollection<TKey, TDocument>
    ...
    where TKey : notnull
{
    ...
    public async Task UpdateManyAsync(IOctoSession session, IEnumerable<TDocument> documents)
    {
        foreach (var document in documents)
        {
            ...
            var result = await _documentCollection.UpdateOneAsync(((IOctoSessionInternal)session).SessionHandle, filterDefinition, updateDefinition);
    ...
    public async Task ReplaceManyAsync(IOctoSession session, IEnumerable<TDocument> documents)
    {
        foreach (var document in documents)
        {
            var id = _mongoDataSourceMapper.GetId(document);
            var filterDefinition = Builders<TDocument>.Filter.BuildIdFilter(id);
            var result = await _documentCollection.ReplaceOneAsync(((IOctoSessionInternal)session).SessionHandle, filterDefinition, document);
            ThrowIfNotAcknowledged(result.IsAcknowledged);
            ThrowIfMatchedCountZero<TDocument>(result.MatchedCount, id);
        }
    }
    ...
    async Task<TDerived?> IDataSourceCollection<TKey, TDocument>.DocumentAsync<TDerived>(IOctoSession session, TKey key) where TDerived : default
    {
        try
        {
            var filter = Builders<TDocument>.Filter.BuildIdFilter(key);
