...
public class Mutation<TEntity> : Engine<TEntity> where TEntity : RtEntity, new()
{
    ...
    public Mutation(IBulkRtMutation bulkRtMutation, IDatabaseContext databaseContext)
    {
    ...
    public async Task ReplaceOneAsync(IOctoSession session, CkId<CkTypeId> ckTypeId, TEntity rtEntity)
    {
        ...
        var rtCollection = _databaseContext.GetRtDatabaseCollection<TEntity>(ckTypeId);
        ...
        await _bulkRtMutation.ApplyChangesAsync(session, _databaseContext, entityUpdateInfoList, 
