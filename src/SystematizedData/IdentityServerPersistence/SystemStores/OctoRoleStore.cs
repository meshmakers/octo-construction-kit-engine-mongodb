    ...
    IQueryableRoleStore<RtRole>
{
    ...
    public OctoRoleStore(ITenantRepository tenantRepository, IdentityErrorDescriber? describer)
    {
    ...
    public async Task<IdentityResult> UpdateAsync(RtRole role, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ThrowIfDisposed();
        string currentConcurrencyStamp = role != null ? role.ConcurrencyStamp : throw new ArgumentNullException(nameof(role));
    ...
    public async Task AddClaimAsync(RtRole role, Claim claim, CancellationToken cancellationToken = default)
    {
        ...
        role.Claims ??= new List<RtRoleClaimRecord>();
